...
int main(int argc, const char *argv[])
{
    ...
    for (int i = 0; i < row/n; i++)
	{
        ...
        resultInt1 = p1_testDataOnDecisionTreeRules(dataArrayInt[i*n]);
        if( resultInt1 != -1){
            predictedClassLabelsInt.push_back(resultInt1);
            continue;
        }
        resultInt2 = p2_testDataOnDecisionTreeRules(dataArrayInt[i*n]);
        if( resultInt2 != -1){
            predictedClassLabelsInt.push_back(resultInt2);
            continue;
        }
        resultInt3 = p3_testDataOnDecisionTreeRules(dataArrayInt[i*n]);
        if( resultInt3 != -1){
            predictedClassLabelsInt.push_back(resultInt3);
            continue;
        }
        resultInt4 = p4_testDataOnDecisionTreeRules(dataArrayInt[i*n]);
        if( resultInt4 != -1){
            predictedClassLabelsInt.push_back(resultInt4);
            continue;
        }
        resultInt5 = p5_testDataOnDecisionTreeRules(dataArrayInt[i*n]);
        if( resultInt5 != -1){
