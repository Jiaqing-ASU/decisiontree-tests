...
int main(int argc, const char *argv[])
{
	...
	int trueReturnArray; //return value if the rules all true
	int falseReturnArray; //return value if at least one rule is false
	...
	if(blockNumber == 1){
		...
		// The function will have a return type of "int32" and take an argument of "int32*".
		Function *F = 
    	...
    	Value *nodeValue = builder.getInt32(valueArray.at(0));
    	Value *trueReturn = builder.getInt32(trueReturnArray);
    	Value *falseReturn = builder.getInt32(falseReturnArray);
    	...
    	Value* givenStart = builder.CreateConstInBoundsGEP1_64(Type::getInt32Ty(Context),givenArray,positionArray.at(0));
    	Value* givenValue = builder.CreateLoad(givenStart);
    	Value *cond;
    	if (compareArray.at(0) == "="){
    		cond = builder.CreateICmp(CmpInst::Predicate::ICMP_EQ, givenValue, nodeValue);
    	}else if (compareArray.at(0) == "!="){
    		cond = builder.CreateICmp(CmpInst::Predicate::ICMP_NE, givenValue, nodeValue);
    	}else if (compareArray.at(0) == ">"){
    		cond = builder.CreateICmp(CmpInst::Predicate::ICMP_UGT, givenValue, nodeValue);
    	}else if (compareArray.at(0) == ">="){
    		cond = builder.CreateICmp(CmpInst::Predicate::ICMP_UGE, givenValue, nodeValue);
    	}else if (compareArray.at(0) == "<"){
    		cond = builder.CreateICmp(CmpInst::Predicate::ICMP_ULT, givenValue, nodeValue);
    	}else if (compareArray.at(0) == "<="){
    		cond = builder.CreateICmp(CmpInst::Predicate::ICMP_ULE, givenValue, nodeValue);
    	}
    	...
    	IRBuilder<> builder(BasicBlockArray.at(0));
 		...
 		// Set some values from the rule.
    	for (int i = 0; i < valueArray.size(); i++){
    		ValueIRArray.push_back(builder.getInt32(valueArray.at(i)));
    	}
    	...
    	for (int i = 0; i < blockNumber-2; i++){
    		...
    		Value* givenStart = builder.CreateConstInBoundsGEP1_64(Type::getInt32Ty(Context),givenArray,positionArray.at(i));
    		Value* givenValue = builder.CreateLoad(givenStart);
    		// Do the compare in the block using corresponding rule.
			Value *cond;
			if (compareArray.at(i) == "="){
    			cond = builder.CreateICmp(CmpInst::Predicate::ICMP_EQ, givenValue, ValueIRArray.at(i));
    		}else if (compareArray.at(i) == "!="){
    			cond = builder.CreateICmp(CmpInst::Predicate::ICMP_NE, givenValue, ValueIRArray.at(i));
    		}else if (compareArray.at(i) == ">"){
    			cond = builder.CreateICmp(CmpInst::Predicate::ICMP_UGT, givenValue, ValueIRArray.at(i));
    		}else if (compareArray.at(i) == ">="){
    			cond = builder.CreateICmp(CmpInst::Predicate::ICMP_UGE, givenValue, ValueIRArray.at(i));
    		}else if (compareArray.at(i) == "<"){
    			cond = builder.CreateICmp(CmpInst::Predicate::ICMP_ULT, givenValue, ValueIRArray.at(i));
    		}else if (compareArray.at(i) == "<="){
    			cond = builder.CreateICmp(CmpInst::Predicate::ICMP_ULE, givenValue, ValueIRArray.at(i));
    		}
    		if (i == blockNumber-3){
